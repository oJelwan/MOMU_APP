...
class F_reportes(tk.Frame):   
    def __init__(self, root,  AutocompleteEntry, hoy, lista_productos, lista_claves, lista_total, venta_db, cot_db, productos_db, compra_db, clien_db, reg_db, reporte_db, *args, **kwargs):
        ...
        self.vt_registro.heading("#11", text = "Pago")
        self.vt_registro.column("#11", width = 70, minwidth = 70)
        self.vt_registro.heading("#12", text = "Lista")
        ...
        # Label TIPO Y FILTROS PUESTOS




    # si se cambia a solo este modulo, habria que ajustar los parametros un poco, o quedarnos con el otro exportar ventas
    def exportar(self):
    ...
    def filtrarVenCot(self, db):
     ...
     # 3. Clave
        try:
            ...
            self.ve_clave.get()[0]  
            try:
                self.reporte_db.fetch()[0]    # Ver si ya hay algo
                for row in self.reporte_db.fetch():
                    if row[2].lower() != self.ve_clave.get().lower():
                        self.reporte_db.remove_product(row[2])
    ...
    def filtrarCompras(self):
     ...
     # 1. No. Venta
        try:
            self.ve_numventa.get()[0]
            for row in self.compra_db.fetch():
                if str(row[1]) == self.ve_numventa.get():
     ...
     # 2. Fecha
        if self.intervalo.get() == 0:
            for row in self.compra_db.inverse_fetch():
                self.reporte_db.insert(row[0], row[1],row[2],row[3],row[4],row[5],row[6], row[7], row[8] , "", "", "", "", "")
        elif self.intervalo.get() != 0:
            try:  # we'll just simply try to convert the string to a Python datetime and see if it fails
                datetime.strptime(self.ve_fecha.get(), '%Y/%m/%d')
            except ValueError:
     ...
     # 5. Proveedor
        try:
            ...
            try:
                ...
                for row in self.compra_db.fetch():
                    if str(row[7]).lower() == self.ve_cliente.get().lower():
                        self.reporte_db.insert(row[0], row[1],row[2],row[3],row[4],row[5],row[6], row[7], row[8] , "", "", "", "", "")
