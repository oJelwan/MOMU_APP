...
class F_Compras(F_Ventas):   
    def __init__(self, root,  AutocompleteEntry, hoy, lista_productos, lista_claves, lista_total, productos_db, compra_db, tmpI1_db, clien_db, reg_db, sucursal, *args, **kwargs):
        super().__init__(root,  AutocompleteEntry, hoy, lista_productos, lista_claves, lista_total, productos_db, compra_db, tmpI1_db, clien_db, sucursal, *args, **kwargs)
    ...
    def historial(self):
        ...
        self.vre_Total = 0
        self.vre_Cant = 0
        for row in self.venta_db.historial():
    ...
    # Variables
     ...
     # 5. Proveedor
        try:
            ...
            self.ie_prov.get()[0]  
            if filtersmt:
                filter += " AND proveedor = " + '"{}"'.format(self.ie_prov.get())
            else:
                filter += "proveedor = " + '"{}"'.format(self.ie_prov.get())
                filtersmt = True
        except:
            pass
...
class F_Orden(tk.Frame):
    ...
    def historial(self):
        self.limpiar()
        for row in self.orden_db.historial():
    ...
    # TODO CAMBIAR FILTROS y quiza paint
    def filtrar(self):
 # Variables
        self.vs_info.set("--")
        self.vre_Cant = 0
