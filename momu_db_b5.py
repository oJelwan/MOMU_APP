...
class Venta_Database:
    ...
    def historial(self):
        self.cur.execute(""" SELECT * FROM ventas ORDER BY registro DESC LIMIT 300""")
        rows = self.cur.fetchall()
        return rows
    ...
    def filter(self, filter):
        f = "SELECT * FROM ventas WHERE "
        f += filter + "COLLATE NOCASE"
        self.cur.execute(f)
        rows = self.cur.fetchall()
        return rows
...
class Cotizacion_Database():
    ...
    def historial(self):
        self.cur.execute(""" SELECT * FROM cotizacion ORDER BY registro DESC LIMIT 300""")
        rows = self.cur.fetchall()
        return rows
...
class Compras_Database:
    ...
    def historial(self):
        self.cur.execute(""" SELECT * FROM compras ORDER BY n_compra DESC LIMIT 300""")
        rows = self.cur.fetchall()
        return rows
    def inverse_fetch(self):
...
class Reporte_Database:
    ...
    def removeu(self, n_compra, clave):
        self.cur.execute("""  DELETE FROM reportes WHERE n_compra = ? AND clave = ?""", (n_compra, clave, ))
        self.conn.commit()
